...
spt_find_page (struct supplemental_page_table *spt UNUSED, void *va UNUSED) {
	...
	mp->va = va;
	struct hash_elem *de = hash_find(&spt->sup_hash,&mp->he);
	struct page *rp = hash_entry(de,struct page,he);
	return rp;
}
...
spt_insert_page (struct supplemental_page_table *spt UNUSED,
	...
	/* TODO: Fill this function. */
	struct hash_elem *result = hash_insert(&spt->sup_hash,&page->he);
	if(result == NULL){
		return true;
	}else{
		return false;
...
supplemental_page_table_init (struct supplemental_page_table *spt UNUSED) {
	hash_init(&spt->sup_hash,hash_page,hash_addr_comp,NULL);
}
...
supplemental_page_table_kill (struct supplemental_page_table *spt UNUSED) {
	...
	hash_destroy(&spt->sup_hash,del_hash_page);
}
