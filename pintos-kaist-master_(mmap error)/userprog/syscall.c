...
#else
struct page* checkaddress(void *cad){
	struct thread *t = thread_current();
	if(is_kernel_vaddr(cad)||cad == NULL||(!spt_find_page(&t->spt,cad)))
		exit(-1);
...
void* mmap(void *addr, size_t length, int writable, int fd, off_t offset){
	if(addr == NULL||length <= 0||is_kernel_vaddr(addr) || is_kernel_vaddr(addr + length)){
		return false;
	}
	...
	return do_mmap(addr,length,writable,df,offset);
}
